...
//==================一个table实例================================//table怎么设置
	  table.render({
	    elem: '#demo',//渲染对象
	    height: 400,//表格高度
	    url: 'queryPartTimePostgraduate', //数据接口
	    where: {key: ''},//给后台传的参数
	    page: true, //开启分页
	    toolbar: '#toolbarDemo',
	    limit: 10,//每页显示信息条数
	...
	$(window).one("resize",function(){//基于框架做的
		$(".add_btn").click(function(){
			var index = layui.layer.open({
				...
				type : 2,
				area: ['800px', '600px'],
				content : "openPartTimePostgraduateAdd",//加载某一张页面
	    ...
	    //do somehing
		 ...
		 //关闭当前提示	
		  ...
		  var index = layui.layer.open({
              ...
              type : 2,
              area: ['800px', '600px'],
              content : "openPartTimePostgraduateEdit?id="+data.id,
	...
	$(window).one("resize",function(){
		$(".newsAdd_btn").click(function(){
			var index = layui.layer.open({
				title : "添加信息",
				type : 2,
				content : "openPartTimePostgraduateAdd",
				success : function(layero, index){
					setTimeout(function(){
						layui.layer.tips('点击此处返回列表', '.layui-layer-setwin .layui-layer-close', {
							tips: 3
